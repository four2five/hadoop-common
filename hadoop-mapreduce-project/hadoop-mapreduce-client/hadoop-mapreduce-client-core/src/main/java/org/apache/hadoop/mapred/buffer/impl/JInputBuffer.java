...
extends Buffer<K, V> implements InputCollector<K, V> {
	...
	class ShuffleRamManager implements RamManager {
		...
		public synchronized boolean reserve(int requestedSize, InputStream ignored) { 
	...
	throws IOException {
		...
		IFileInputStream checksumIn = new IFileInputStream(ins, (long)compressedLength, conf);
		ins = checksumIn;       
	...
	createKVIterator(JobConf job, FileSystem fs, Reporter reporter) throws IOException {
		...
		if (0 != onDiskBytes) {
			...
			riter = Merger.merge(job, fs, keyClass, valClass,
					...
					comparator, reporter, null, null, null);
		}
