...
class DamascEventFetcher<K,V> extends Thread {
  private static final long SLEEP_TIME = 1000;
  private static final int MAX_RETRIES = 10;
  private static final int RETRY_PERIOD = 5000;
  private static final Log LOG = LogFactory.getLog(DamascEventFetcher.class);
  ...
  public DamascEventFetcher(TaskAttemptID reduce,
                      TaskUmbilicalProtocol umbilical,
                      ShuffleScheduler<K,V> scheduler,
                      ExceptionReporter reporter,
                      int maxEventsToFetch,
                      int[] mapTaskDependencies) {
    setName("DamascEventFetcher for fetching Map Completion Events");
    setDaemon(true);    
    this.reduce = reduce;
    ...
    try {
      while (!stopped && !Thread.currentThread().isInterrupted()) {
        try {
          ...
          LOG.info("DamascEventFetcher is interrupted.. Returning");
          return;
  ...
  private boolean reducerCaresAboutThisMapTask(TaskID task) { 
