...
public class FileHandle {
  ...
  private static final String jobDir =  "fileHandle/";  
  ...
  public Path getOutputFile(TaskAttemptID taskid)
    throws IOException {
    return lDirAlloc.getLocalPathToRead(getIntermediateOutputDir(
                       this.conf.getUser(),
  ...
  public Path getOutputFileForWrite(TaskAttemptID taskid, long size)
    throws IOException {
    return lDirAlloc.getLocalPathForWrite(getIntermediateOutputDir(
                       this.conf.getUser(),
                       jobId.toString(), taskid.toString(),
                       TaskType.TASK_CLEANUP == taskid.getTaskType()) + 
                       "/file.out", size, conf);
  ...
  throws IOException {
    return lDirAlloc.getLocalPathForWrite(getIntermediateOutputDir(
                       ...
                       TaskType.TASK_CLEANUP == attempt.getTaskType()) + 
   ...
   */
  public Path getInputFile(TaskAttemptID reduceTaskId, TaskID mapTaskId, int id)
    throws IOException {
    return lDirAlloc.getLocalPathToRead(getIntermediateOutputDir(
                       this.conf.getUser(),
                       jobId.toString(), reduceTaskId.toString(),
                       TaskType.TASK_CLEANUP == reduceTaskId.getTaskType()) + 
   ...
   * @param size the size of the file
   */
  public Path getInputFileForWrite(TaskAttemptID reduceTaskId, TaskID mapTaskId, int id, long size)
    throws IOException {
    return lDirAlloc.getLocalPathForWrite(getIntermediateOutputDir(
                       this.conf.getUser(),
                       jobId.toString(), reduceTaskId.toString(),
  ...
  public void removeAll(TaskAttemptID taskid) throws IOException {
    conf.deleteLocalFiles(getIntermediateOutputDir(
                       this.conf.getUser(),
                          jobId.toString(), taskid.toString(), 
                          TaskType.TASK_CLEANUP == taskid.getTaskType()));
    
    conf.deleteLocalFiles(getIntermediateInputDir(
                          this.conf.getUser(),
                          jobId.toString(), taskid.toString(),
                          TaskType.TASK_CLEANUP == taskid.getTaskType()));
  }
  ...
  public static String getIntermediateInputDir(String user, String jobid, 
                                               String taskid, boolean isCleanupAttempt) {
    return getLocalTaskDir(user, jobid, taskid, isCleanupAttempt) 
            + Path.SEPARATOR + FileHandle.INPUT ; 
  }

  static String getIntermediateOutputDir(String user, String jobid, String taskid, boolean isCleanupAttempt) {
    return getLocalTaskDir(user, jobid, taskid, isCleanupAttempt)
           + Path.SEPARATOR + FileHandle.OUTPUT ;
  }
