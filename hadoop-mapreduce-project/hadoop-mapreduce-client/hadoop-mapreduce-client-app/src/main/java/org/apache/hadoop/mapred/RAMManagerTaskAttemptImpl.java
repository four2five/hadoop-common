...
public class RAMManagerTaskAttemptImpl extends TaskAttemptImpl {
  ...
  public Task createRemoteTask() {
    ...
    RAMManagerTask ramManagerTask =
      new RAMManagerTask("", TypeConverter.fromYarn(getID()), partition,
          nodeId, 1); // YARN doesn't have the concept of slots per task, set it as 1.
    ramManagerTask.setUser(conf.get(MRJobConfig.USER_NAME));
    ramManagerTask.setConf(conf);
    return ramManagerTask;
  }
