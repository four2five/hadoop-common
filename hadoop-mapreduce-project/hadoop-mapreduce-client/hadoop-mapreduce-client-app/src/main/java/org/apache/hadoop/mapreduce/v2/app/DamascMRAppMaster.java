...
import org.apache.hadoop.mapreduce.v2.app.job.impl.DamascJobImpl;
...
public class DamascMRAppMaster extends CompositeService {

  private static final Log LOG = LogFactory.getLog(DamascMRAppMaster.class);

  /**
   * Priority of the DamascMRAppMaster shutdown hook.
   */
  ...
  public DamascMRAppMaster(ApplicationAttemptId applicationAttemptId,
  ...
  public DamascMRAppMaster(ApplicationAttemptId applicationAttemptId,
    ...
    super(DamascMRAppMaster.class.getName());
    this.clock = clock;
    ...
    LOG.info("Created DamascMRAppMaster for application " + applicationAttemptId);
  }
  ...
  public void shutDownJob() {
    ...
    try {
      ...
      if ( !isLastAMRetry){
        if (((DamascJobImpl)job).getInternalState() != JobStateInternal.REBOOT) {
  ...
  protected Job createJob(Configuration conf, JobStateInternal forcedState, 
    ...
    // create single job
    Job newJob =
        new DamascJobImpl(jobId, appAttemptID, conf, dispatcher.getEventHandler(),
  ...
  protected void serviceStart() throws Exception {
    ...
    DefaultMetricsSystem.initialize("DamascMRAppMaster");
    ...
    if (!errorHappenedShutDown) {
      ...
      // DamascJobImpl's InitTransition is done (call above is synchronous), so the
      ...
      if (job.isUber()) {
        ...
        LOG.info("DamascMRAppMaster uberizing job " + job.getID()
        ...
        LOG.info("DamascMRAppMaster launching normal, non-uberized, multi-container "
            + "job " + job.getID() + ".");
      }
  ...
  public static void main(String[] args) {
    try {
      ...
      DamascMRAppMaster appMaster =
          new DamascMRAppMaster(applicationAttemptId, containerId, nodeHostString,
              Integer.parseInt(nodePortString),
              Integer.parseInt(nodeHttpPortString), appSubmitTime,
              Integer.parseInt(maxAppAttempts));
      ShutdownHookManager.get().addShutdownHook(
        new DamascMRAppMasterShutdownHook(appMaster), SHUTDOWN_HOOK_PRIORITY);
      ...
      LOG.fatal("Error starting DamascMRAppMaster", t);
      System.exit(1);
    }
  }
  ...
  // close of the JVM.
  static class DamascMRAppMasterShutdownHook implements Runnable {
    DamascMRAppMaster appMaster;
    DamascMRAppMasterShutdownHook(DamascMRAppMaster appMaster) {
      this.appMaster = appMaster;
    }
    public void run() {
      LOG.info("DamascMRAppMaster received a signal. Signaling RMCommunicator and "
  ...
  protected static void initAndStartAppMaster(final DamascMRAppMaster appMaster,
