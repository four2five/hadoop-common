...
public class HadoopUtils {
    ...
    //private static boolean partitionerClassSet = false;
    ...
    //private static boolean cephConfPathSet = false;
    //private static boolean outputDataTypeSizeSet = false;
    ...
    public static void setVariableShape(Configuration conf, int[] inVariableShape) {
      // don't allow setting this once it is set 
      /*
      if( variableShapeSet ) { 
        return;
      }
    ...
    */
    ...
    public static long getReducerKeyLimit(Configuration conf) {
        //if ( !reducerKeyLimitSet) {
            reducerKeyLimit = 
              conf.getLong(REDUCER_KEY_LIMIT, 
                           (long)-1);
         //   reducerKeyLimitSet = true;
        //}
  ...
  /*
  public static int determineRecordDimension(int[] stepShape) { 
    // reasonable default value
    int recordDimension = stepShape.length - 1;

    // dimension lenth 1 means no length at all, unless it's the 
    // highest dimension, then it's a single cell's length of data
    for( int i=0; i<stepShape.length; i++) { 
      if( 1 != stepShape[i] ) { 
        recordDimension = i;
        break;
      }
  ...
  */
  ...
  /*
  public static int determineRecordDimension(int[] stepShape, int minNumSteps ) { 
    // reasonable default value
    int recordDimension = stepShape.length - 1;
    int higherLevelSteps = 1;

    // dimension lenth 1 means no length at all, unless it's the 
    // highest dimension, then it's a single cell's length of data
    for( int i=0; i<stepShape.length; i++) { 
      if( (1 != stepShape[i]) & ((stepShape[i] * higherLevelSteps) > minNumSteps)) { 
        recordDimension = i;
        break;
      }

      higherLevelSteps *= stepShape[i];
    }
  ...
  */
  ...
  public static int[] getReducerWriteShape( int reducerNum,
    ...
    //LOG.debug("JB, debug print: " + Arrays.toString(returnShape));

    long[] strides = computeStrides(totalOutputSpace);
    //returnShape[recordDim] = (int) (recordStepSize / strides[recordDim]);
    returnShape[recordDim] = recordStepSize;

    //LOG.debug("JB, debug print2: " + Arrays.toString(returnShape));
    ...
    if( (numReducers-1) == reducerNum) { 
      ...
      //returnShape = correctArray(returnCorner, returnShape, totalOutputSpace);
  ...
  */
  ...
  {
  /*
    LOG.debug("increment start: " + Utils.arrayToString(currentShape) + 
                       " increment shape: " + Utils.arrayToString(incrementShape) + 
                       " var shape: " + Utils.arrayToString(varShape) );
  */
    ...
    //LOG.debug("increment middle: " + Utils.arrayToString(currentShape));
    ...
    //LOG.debug("increment end: " + Utils.arrayToString(currentShape));
    return currentShape;
  }
