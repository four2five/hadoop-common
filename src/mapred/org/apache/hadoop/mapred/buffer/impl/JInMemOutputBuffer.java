...
public class JInMemOutputBuffer<K extends Object, V extends Object> 
       extends Buffer<K, V>
  ...
  */
	...
	private class SpillThread extends Thread {
		...
		public void close() throws IOException {
            ...
            ByteArrayOutputStream baIndexOut = null;
            baOut = new ByteArrayOutputStream(dataSize);
            baIndexOut = new ByteArrayOutputStream(partitions * MAP_OUTPUT_INDEX_RECORD_LENGTH);
		...
		private void pipeline() throws IOException {
			...
			if (numSpillFiles == 1) {
				InMemoryPartitionBuffer spill = spills.get(nextPipelineSpill);
				OutputInMemoryBuffer buffer = new OutputInMemoryBuffer(taskid, nextPipelineSpill, spill.progress,
						                        spill.data.toByteArray(), spill.index.toByteArray(), spill.eof, partitions);
				LOG.info("JB, " +  taskid + " pipelining spill file " + nextPipelineSpill + 
                 "progress " + spill.progress + " eof " + spill.eof);
				umbilical.output(buffer);
				nextPipelineSpill++;
	...
	private class InMemoryPartitionBuffer {
		int id;
    ...
    public int DEFAULT_BYTE_BUFFER_SIZE = 1024 * 1024 * 100; // 100 MB
		
		public InMemoryPartitionBuffer(int id, ByteArrayOutputStream data, ByteArrayOutputStream index, float progress, boolean eof) {
			this.id = id;
			this.data = data;
			this.index = index;
			this.valid = true;
		...
		public String toString() {
			return this.id + "[" + dataSize() + " bytes]";
		}

		public void copy(InMemoryPartitionBuffer buffer) throws IOException {
			if (!buffer.valid()) {
				throw new IOException("InMemoryParitionBuffer : copy from an unvalid file!");
			}
			this.id = buffer.id;
			this.progress = buffer.progress;
			this.eof = buffer.eof;
			this.valid = true;
	...
	private static final Log LOG = LogFactory.getLog(JInMemOutputBuffer.class.getName());
	...
	private final InMemoryBufferUmbilicalProtocol umbilical;
	...
	private static final int RECREP = 3;  // total #fields in acct
	...
	// spill accounting
	private List<InMemoryPartitionBuffer> spills = new ArrayList<InMemoryPartitionBuffer>();
  ...
  private int totalKeys;
	...
	@SuppressWarnings("unchecked")
	public JInMemOutputBuffer(InMemoryBufferUmbilicalProtocol umbilical, Task task, JobConf job, 
		...
		sorter = (IndexedSorter) ReflectionUtils.newInstance(job.getClass("map.sort.class", QuickSort.class), job);
  ...
  public void close() throws IOException, InterruptedException { 
	...
	public synchronized OutputInMemoryBuffer oldClose() throws IOException {
		LOG.info("JInMemOutputBuffer: closed called at progress " + progress.get());
		this.eof = true;
		OutputInMemoryBuffer finalOutput = innerFlush();
		return finalOutput;
	}
  ...
  */
    ...
    this.totalKeys++;
      ...
      kvindices[ind + RECREP] = (int)recordsRepresented;
	...
	/**
	 * Inner class managing the spill of serialized records to a ByteBuffer.
	 */
  ...
  public void flush() throws IOException, InterruptedException,
	...
	public OutputInMemoryBuffer innerFlush() throws IOException {
	...
	private float sortAndSpill() throws IOException {
      ...
      ByteArrayOutputStream baIndexOut = null;
        ...
        */
        ...
        // use a ByteBuffer
        baOut = new ByteArrayOutputStream((int)filesize);
        baIndexOut = new ByteArrayOutputStream(partitions * MAP_OUTPUT_INDEX_RECORD_LENGTH);
        out = new DataOutputStream(baOut);
        indexOut = new DataOutputStream(baIndexOut);
        // create an JInMemoryOutputBuffer instead of a spill file
				if (out == null ) throw new IOException("Unable to create JInMemOutputFile");
				// create spill index
				...
				sorter.sort(JInMemOutputBuffer.this, kvstart, endPosition, reporter);
				int spindex = kvstart;
				...
				return combinerClass != null && rawDataSize > 0 ? 
						(float) out.size() / (float) rawDataSize : 1f;
			} finally {
				if (out != null) { 
          out.close();
          baOut.close();
	...
	private void spillSingleRecord(final K key, final V value)  throws IOException {
      ...
      ByteArrayOutputStream baIndexOut = null;
			...
			try {
        baOut = new ByteArrayOutputStream((int)size);
        baIndexOut = new ByteArrayOutputStream(partitions * MAP_OUTPUT_INDEX_RECORD_LENGTH);
        out = new DataOutputStream(baOut);
        indexOut = new DataOutputStream(baIndexOut);
        /*
    ...
    public int getNumRecordsRepresented() throws IOException {
      final int kvoff = kvoffsets[current % kvoffsets.length];
      LOG.info("In JInMemOutputBuffer, getNumRecordsRepresented: " + kvindices[kvoff + RECREP]);
      return kvindices[kvoff + RECREP];
    }
  ...
  */
	...
	private void writeIndexRecord(DataOutputStream indexOut,
			DataOutputStream out, long start,
			IFile.Writer<K, V> writer)
	throws IOException {
		...
		long segmentLength = out.size() - start;
